...
def get_feature(person, im_num):
    if os.path.exists(f'./prenn/{person}/{im_num}' + '.png'):        # Judge image type
        file = f'./prenn/{person}/{im_num}'+'.png'
    elif os.path.exists(f'./prenn/{person}/{im_num}' + '.jpg'):
        file = f'./prenn/{person}/{im_num}'+'.jpg'
...
def get_features(typ, first, last):
    ...
    curv = []
    num_conn = []
    for i in range(first, last + 1):  # Volunteer number
        winding.append([])
        curv.append([])
        num_conn.append([])
        for j in range(10 * typ + 1, 10 * typ + 11):
            if (not os.path.exists(f'./prenn/{i}/{j}'+'.png')) and (not os.path.exists(f'./prenn/{i}/{j}'+'.jpg')):
                continue
...
if __name__ == '__main__':
    ...
    for i in range(4):
        ...
        print(f'{(i + 1)/4*100}% Finished')
    end = time.time()
    pool.close()
    print(end-start)
    #Save data
    with open('num_conn.dat', 'w') as f:
        json.dump(num_conn, f)
    with open('winding.dat', 'w') as f:
        json.dump(winding, f)
    with open('curv.dat', 'w') as f:
        json.dump(curv, f)